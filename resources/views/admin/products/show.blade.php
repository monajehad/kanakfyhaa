        ...
        <div class="card-body">
            <div class="row g-4 align-items-center">
                ...
                <div class="col-lg-4 text-center mb-4 mb-lg-0">
                    ...
                    @endphp
                    @if($mainImage)
                        <img src="{{ $mainImage->url }}" alt="{{ $mainImage->alt_text ?? $product->name }}" class="rounded border" style="max-width: 95%; max-height: 270px;">
                        @if($mainImage->alt_text)
                            <small class="d-block mt-1 text-muted">{{ $mainImage->alt_text }}</small>
                        @endif
                    @else
                        <div class="text-center text-muted" style="min-height:180px;display:flex;align-items:center;justify-content:center;">
                        ...
                        <div class="mt-3">
                            ...
                            <div class="d-flex flex-wrap gap-2 justify-content-center mt-2">
                                @foreach($subImages as $media)
                                    <div>
                                        <img src="{{ $media->url }}" alt="{{ $media->alt_text ?? $product->name }}" style="max-width: 65px; max-height: 65px; border:1px solid #ddd; border-radius:5px; padding:1px;">
                                        @if($media->alt_text)
                                            <div class="text-xs text-muted pt-1">{{ \Illuminate\Support\Str::limit($media->alt_text, 13) }}</div>
                                        @endif
                                    </div>
                                @endforeach
                            </div>
                    ...
                    <dl class="row mb-0 gy-3">
                        ...
                        <dd class="col-sm-8">
                            @if(is_array($product->sizes) && count($product->sizes))
                                <span class="badge bg-light text-dark fw-normal">{{ implode('</span> <span class="badge bg-light text-dark fw-normal">', $product->sizes) }}</span>
                            @else
                                -
                            @endif
                        </dd>
                        ...
                        <dd class="col-sm-8"><span class="text-monospace dir-ltr user-select-all" style="font-size:smaller">{{ $product->uuid }}</span></dd>

                        <dt class="col-sm-4">رمز QR</dt>
                        <dd class="col-sm-8">
                            @if($product->qr_code)
                                <img src="{{ asset('storage/' . $product->qr_code) }}" alt="رمز QR" width="90" style="background:#fafbfc;padding:7px;border-radius:8px">
                            @else
